...
MooEditable.UI.TableDialog = function(editor, dialog){
    var html = {
        ...
        tableedit: MooEditable.Locale.get('tableWidth') + ' <input type="text" class="table-w" value="" size="4"> '
            ...
            + MooEditable.Locale.get('tableBorder') + ' <input type="text" class="table-b" value="" size="15"> ',
    ...
    var action = {
        ...
        tableedit: {
            load: function(e){
                ...
                this.el.getElement('.table-b').set('value', node.get('border'));
            },
            click: function(e){
                var node = editor.selection.getNode().getParent('table');
                node.set('width', this.el.getElement('.table-w').value);
                node.set('border', this.el.getElement('.table-b').value);
